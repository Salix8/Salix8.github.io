...
VERSION_NUMBER = /* 5ETOOLS_VERSION__OPEN */"1.97.1"/* 5ETOOLS_VERSION__CLOSE */;
...
Parser.SOURCE_JSON_TO_FULL[SRC_GPNM] = "Guía de Peter a la No tan Magia";
Parser.SOURCE_JSON_TO_FULL[SRC_GMAM] = "Guía de Molly a las Artes de Maztica";
Parser.SOURCE_JSON_TO_FULL[SRC_CdS] = "Compendio de los Sueños";
Parser.SOURCE_JSON_TO_FULL[SRC_GHLoE] = "Grim Hollow: Lairs of Etharis";
...
Parser.SOURCE_JSON_TO_ABV[SRC_GPNM] = "GPNM";
Parser.SOURCE_JSON_TO_ABV[SRC_GMAM] = "GMAM";
Parser.SOURCE_JSON_TO_ABV[SRC_CdS] = "CdS";
Parser.SOURCE_JSON_TO_ABV[SRC_GHLoE] = "GHLoE";
...
Parser.SOURCE_JSON_TO_DATE[SRC_BMT] = "2023-11-14";
Parser.SOURCE_JSON_TO_DATE[SRC_EGW] = "2020-03-17";
Parser.SOURCE_JSON_TO_DATE[SRC_FTD] = "2021-10-26";
Parser.SOURCE_JSON_TO_DATE[SRC_IDRotF] = "2020-09-15";
Parser.SOURCE_JSON_TO_DATE[SRC_SatO] = "2023-10-17";
...
ListUtil = {
	...
	_firstInit: true,
	initList (listOpts) {
		const $iptSearch = listOpts.$iptSearch || $("#lst__search");
		const $wrpList = listOpts.$wrpList || $(`ul.list.${listOpts.listClass}`);
		const listName = listOpts.listClass || ($wrpList.length ? $wrpList.attr("class") : "unknown");
		if (!$iptSearch.length) throw new Error(`Could not initialise list "${listName}": missing #lst__search`);
		if (!$wrpList.length) throw new Error(`Could not initialise list "${listName}": missing list wrapper`);
...
UrlUtil.PG_MAKE_SHAPED = "__removed_makeshaped";
UrlUtil.PG_MANAGE_BREW = "managebrew.html";
UrlUtil.PG_MAKE_BREW = "makebrew.html";
UrlUtil.PG_DEMO_RENDER = "__removed_renderdemo";
...
DataUtil = {
	...
	async _pLoad (url) {
		...
		DataUtil._loading[url] = new Promise((resolve, reject) => {
			...
			request.onload = function () {
				try {
					if (this.status && this.status >= 400) throw new Error(`HTTP ${this.status}`);
					if (!this.response || !this.response.trim()) throw new Error("empty response");
	...
	class: {
		_pLoadingJson: null,
		_loadedJson: null,
		_getFeatureKey: function (...parts) {
			return parts.join("|").toLowerCase();
		},
		_getRefLevel: function (ref) {
			...
			if (ref.subclassFeature) {
				const parts = ref.subclassFeature.split("|");
				return Number(parts[parts.length - 1]);
			}
			return Number(ref.level || 1);
		},
		_getCleanFeature: function (feature) {
			const out = MiscUtil.copy(feature);
			delete out.className;
			delete out.classSource;
			delete out.subclassName;
			delete out.subclassShortName;
			delete out.subclassSource;
			delete out.level;
			return out;
		},
		_getGroupedFeatures: function (refs, featureLookup, derefProp) {
			if (!refs) return refs;
			if (refs.every(it => it instanceof Array)) return refs;

			const out = [...new Array(20)].map(() => []);
			refs.forEach(ref => {
				const level = DataUtil.class._getRefLevel(ref);
				if (!level || level < 1 || level > 20) throw new Error(`Invalid class feature level in reference: ${JSON.stringify(ref)}`);

				let feature;
				if (typeof ref === "string") feature = featureLookup[ref.toLowerCase()];
				else if (ref[derefProp]) feature = featureLookup[ref[derefProp].toLowerCase()];
				else feature = ref;

				if (!feature) throw new Error(`Could not dereference class feature: ${JSON.stringify(ref)}`);
				const cleanFeature = DataUtil.class._getCleanFeature(feature);
				if (ref.gainSubclassFeature) cleanFeature.gainSubclassFeature = true;
				out[level - 1].push(cleanFeature);
			});
			return out;
		},
		_mutPostLoad: function (data) {
			if (!data || (!data.classFeature && !data.subclassFeature)) return data;

			const classFeatureLookup = {};
			(data.classFeature || []).forEach(feature => {
				classFeatureLookup[DataUtil.class._getFeatureKey(feature.name, feature.className, feature.classSource, feature.level)] = feature;
			});

			const subclassFeatureLookup = {};
			(data.subclassFeature || []).forEach(feature => {
				subclassFeatureLookup[DataUtil.class._getFeatureKey(feature.name, feature.className, feature.classSource, feature.subclassShortName || feature.subclassName, feature.subclassSource, feature.level)] = feature;
			});

			(data.class || []).forEach(cls => {
				cls.classFeatures = DataUtil.class._getGroupedFeatures(cls.classFeatures, classFeatureLookup, "classFeature");
				cls.subclasses = cls.subclasses || [];
				(cls.classTableGroups || []).forEach(tableGroup => {
					if (!tableGroup.rows && tableGroup.rowsSpellProgression) tableGroup.rows = tableGroup.rowsSpellProgression;
				});
			});

			const remainingSubclasses = [];
			(data.subclass || []).forEach(sc => {
				sc.subclassFeatures = DataUtil.class._getGroupedFeatures(sc.subclassFeatures, subclassFeatureLookup, "subclassFeature");

				if (sc.className) {
					const cls = (data.class || []).find(it => it.name === sc.className && it.source === (sc.classSource || SRC_PHB));
					if (cls) {
						const cleanSc = MiscUtil.copy(sc);
						delete cleanSc.className;
						delete cleanSc.classSource;
						cls.subclasses = cls.subclasses || [];
						cls.subclasses.push(cleanSc);
						return;
					}
				}

				if (sc.className && !sc.class) sc.class = sc.className;
				remainingSubclasses.push(sc);
			});

			data.subclass = remainingSubclasses;
			delete data.classFeature;
		...
		loadJSON: async function (baseUrl = "") {
			...
			DataUtil.class._pLoadingJson = (async () => {
				...
				const classFiles = Object.values(index || {}).filter(Boolean);
				if (!classFiles.length) throw new Error(`Class index did not contain any loadable files: ${baseUrl}data/class/index.json`);

				const allData = await Promise.all(classFiles.map(async file => {
					const data = await DataUtil.loadJSON(`${baseUrl}data/class/${file}`);
					if (!data.class || !data.class.length) throw new Error(`Class file did not contain class entries: ${baseUrl}data/class/${file}`);
					return DataUtil.class._mutPostLoad(data);
				}));
