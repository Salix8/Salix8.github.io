...
class MultiSource {
	...
	async pMultisourceLoad (jsonDir, filterBox, pPageInit, addFn, pOptional) {
		...
		const defaultSel = sources.filter(s => PageFilter.defaultSourceSelFn(s) && src2UrlMap[s]);
		const fallbackSources = defaultSel.length ? defaultSel : sources.filter(s => src2UrlMap[s]).slice(0, 1);
		...
		if (allSources.length === 0) {
			...
			fallbackSources.forEach(src => allSources.push(src));
		}
		...
		if (allSources.length === 0) {
			const message = `No local data sources were available for ${jsonDir}index.json`;
		...
		if (toLoads.length > 0) {
			...
			const dataStack = (await Promise.all(toLoads.map(async toLoad => {
				try {
					...
					loadErrors.push(`${toLoad.src}: ${e.message}`);
					return null;
				}
			}))).filter(Boolean).flat();
			...
			if (!toAdd.length) {
				const message = `Loaded ${jsonDir}index.json, but no ${this._prop} entries were produced.${loadErrors.length ? ` Errors: ${loadErrors.join("; ")}` : ""}`;
				if (typeof JqueryUtil !== "undefined") JqueryUtil.doToast({type: "danger", content: message});
				throw new Error(message);
			}
			addFn(toAdd);
