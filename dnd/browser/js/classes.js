...
class ClassesPage extends BaseComponent {
	...
	constructor () {
		...
		this._isRecoveringFromEmptyList = false;
		// endregion
	}
	...
	async pOnLoad () {
		...
		if (!data.class || !data.class.length) {
			const message = "Loaded data/class/index.json, but no class entries were produced.";
	...
	_addData (data) {
		data = DataUtil.class._mutPostLoad(data);
	...
	_handleFilterChange () {
		...
		if (
			!this._isInitialLoadComplete
			&& !this._isRecoveringFromEmptyList
			&& this._list.items.length
			&& !this._list.visibleItems.length
		) {
			this._isRecoveringFromEmptyList = true;
			$(`#lst__search`).val("");
			this._list.search("");
			this._filterBox.reset(true);
			this._isRecoveringFromEmptyList = false;
			return;
		}

		if (!this._isInitialLoadComplete && this._list.items.length && !this._list.visibleItems.length) {
			const excludedCount = this._list.items.filter(it => it.data.isExcluded).length;
			JqueryUtil.doToast({
				type: "danger",
				content: `Classes data loaded (${this._dataList.length}), but no classes are visible. Excluded: ${excludedCount}. Use Settings > Reset Local State if this persists.`
			});
		}
...
window.addEventListener("load", () => classesPage.pOnLoad().catch(e => {
	console.error(e);
	const message = `Could not initialise Classes page: ${e.message}`;
	if (typeof JqueryUtil !== "undefined") JqueryUtil.doToast({type: "danger", content: message});
	$(`#pagecontent`).html(`<tr><th class="border" colspan="6"></th></tr><tr><td colspan="6" class="text-danger">${message}</td></tr><tr><th class="border" colspan="6"></th></tr>`);
}));
